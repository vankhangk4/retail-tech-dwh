              ...
              {loading ? (
                [1, 2, 3].map((i) => (
                  <tr key={i}>
                    ...
                    <td><div className="skeleton" style={{ width: 70, height: 28, borderRadius: 6 }} /></td>
                  </tr>
                ))
              ) : users.map((u) => (
                <tr key={u.UserId}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setForm({ ...form, Role: newRole, TenantId: newRole === 'SuperAdmin' ? '' : form.TenantId });
                  }}
              ...
              <div className="form-group">
                ...
                {form.Role === 'SuperAdmin' ? (
                  <input type="text" value="SuperAdmin" className="form-input" disabled style={{ background: '#f1f5f9', color: '#059669', fontWeight: 600 }} />
                ) : (
                  <select value={form.TenantId} onChange={(e) => setForm({ ...form, TenantId: e.target.value })} className="form-select" required>
                    <option value="">-- Chọn Tenant --</option>
                    {tenants.map((t) => (
                      <option key={t.TenantId} value={t.TenantId}>{t.TenantId} - {t.TenantName}</option>
                    ))}
                  </select>
                )}
              </div>
